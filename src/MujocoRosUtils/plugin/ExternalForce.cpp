...
)
{
  ...
  mju_mulMatVec3(pos_world, d->xmat + 9*body_id_, pos_local);
...
)
{
  ...
  mjtNum from[3] = { pos_world[0], pos_world[1], pos_world[2] };
  mjtNum to[3]   = { arrow_end[0],  arrow_end[1],  arrow_end[2]  };

  // mjv_initGeom should have been called already on *force_geom*
  mjv_connector(force_geom, mjGEOM_ARROW, width, from, to);
